...
class TraceClusterDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        for trace_id, trace in tqdm(raw_data.items()):
            ...
            edge_index = self._get_adjacency_info(trace)       # from to 有问题，[from1, from2, from3 ...] [to1, to2, to3 ...]
    ...
    def _get_adjacency_info(self, trace):
        ...
        for from_id, to_list in trace["edges"]:
            for to in to_list:
                to_id = to["spanId"]
                adj_list.append([from_id, to_id])    # 1.数据格式有问题，torch.tensor，dtype=torch.long    2.[2, num_edge] 维度不对
