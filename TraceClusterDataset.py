...
class TraceClusterDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        for trace_id, trace in tqdm(raw_data.items()):
            ...
            num_nodes_edge_index = edge_index.max()+2    # 包括 0 和 -1
            if num_nodes_node_feats != num_nodes_edge_index:
                print("Feature dismatch! num_nodes_node_feats: {}, num_nodes_edge_index: {}".format(num_nodes_node_feats, num_nodes_edge_index))
    ...
    def _get_node_features(self, trace):
        ...
        """
        node_feats = []
        for span_id, attr in sorted(trace["vertexs"]):    # 一个 span 一个点，一个点一行
            feat = []
            feat.append(attr)
    ...
    def get(self, idx):
        ...
        """
        node_num = data.edge_index.max()     # ？？？？？？？ 真的不用+1吗
