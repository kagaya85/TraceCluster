...
def main():
    model_path = 'E:\Data\TraceCluster\log_node\\20epoch_GIN_add_0.5_view_random\\'
    batch_size = 128
    normal_classes = [0]
    abnormal_classes = [1]
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    normal_dataset = TraceDataset(root='E:\Data\TraceCluster\\0218_node_data\\train_data_mem', aug='none')
    test_dataset = TraceDataset(root='E:\Data\TraceCluster\\0218_node_data\\test_data_mem', aug='none')
    ...
    train_dataset = Subset(normal_dataset, train_idx)
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
    eval_dataloader = DataLoader(test_dataset, batch_size=batch_size, shuffle=True)

    model = SIMCLR(num_layers=num_layers, input_dim=normal_dataset.num_node_features, output_dim=output_dim,
                   num_edge_attr=normal_dataset.num_edge_features, gnn_type=gnn_type,
