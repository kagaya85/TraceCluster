...
def load_dataset():
    ...
    with open(r'G:/workspace/TraceCluster/preprocessNew/normal.json', 'r') as file_1:
    ...
    with open(r'G:/workspace/TraceCluster/preprocessNew/chaos.json', 'r') as file_2:
...
def process_one_trace(trace, unique_path, all_trace):
    ...
    return unique_path, all_trace


def embedding_to_vector(trace_results, unique_path):
    length = len(unique_path)
    trace_vector = []
    for traceItem in trace_results:
        vector_list = [-1 for i in range(length)]
        for i in range(1, len(traceItem['service_seq'])):
            vector_list[unique_path.index('->'.join(traceItem['service_seq'][:i + 1]))] = traceItem['time_seq'][i-1]
        trace_vector.append(vector_list)
    return trace_vector