...
class RCTree:
    ...
    def _cut(self, X, S, parent=None, side='l'):
        # Find all invalid dimensions
        xmax = X[S].max(axis=0)
        xmin = X[S].min(axis=0)
        invalid_dims = np.where(xmin==-1)[0]
        # If there are invalid dimensions
        while invalid_dims.size != 0:
            # Determine dimension to cut
            q = self.rng.choice(invalid_dims)
            if X[S].sum(0)[q] == -1*len(X[S]):
                invalid_dims = np.delete(invalid_dims, list(invalid_dims).index(q))
            else:
                break    
        # Determine value for split
        p = -0.5
    ...
    # 用于 construction stage RRCF 的构建
    ...
    # 照 maintenance stage 的流程改进，需添加: 3.缓解唯独灾难的步骤（每次添加新节点都检查是否存在 weak dimension，若有则移除所有节点的该dimension，同时要时刻监控被移除的dimension，若恢复正常则需添加该dimension）
    ...
    # disp 和 codisp 换成基于 attention score 计算采样概率的方法
    def disp(self, leaf):
        """
    ...
    # 应改为用 invalid dimension 或者 weight 来计算 cut dimension 和 cut value
